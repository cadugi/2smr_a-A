...
# Obtén la ubicación del directorio actual del script
...
def guardar_contrasena(usuario, contrasena):
    with open(archivo_contrasenas, "a", newline='') as archivo:  # Modo de apertura "a" para añadir al archivo
        escritor = csv.writer(archivo)
        escritor.writerow([usuario, contrasena.decode()])

def ver_contraseñas():
    try:
        with open(archivo_contrasenas, "r", newline='') as archivo:  # Modo de apertura "r" para lectura
...
while True:
    ...
    if opcion == "g":
        ...
        print("Contraseña generada y almacenada en el archivo contrasenas.csv.")
    elif opcion == "v":
        ver_contraseñas()
    elif opcion == "d":
